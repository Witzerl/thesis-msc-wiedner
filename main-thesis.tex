...
\usepackage{csquotes}
\usepackage[backend=biber,style=authoryear,natbib=true,sortcites=true,maxcitenames=2]{biblatex}
...
% TODO: finalize thesis title with supervisor.
\title{Neural PDE Solvers for Geographic Atrophy Progression Prediction from Longitudinal OCT Imaging}

%% Command \titleshort{short title}: sets an abbreviated version of the thesis title for page heads
\titleshort{Neural PDE Solvers for GA Progression Prediction}
...
% TODO: fill in author name, academic prefix/suffix, and matriculation number.
\author{Christian~Wiedner\matno{K52001455}}
...
\supervisor{\prefix{Assoc.~Prof. PD~Dipl.-Ing. Dr.} Hrvoje~Bogunovi\'c}

%% Command \assistantsupervisor{name}: sets the name of the assistant supervisor(s)
\assistantsupervisor{\prefix{Dr.} Dmitrii~Lachinov}

%% Command \degree{degree}{degree program}: sets the degree and degree program name
% TODO: confirm exact degree program name with JKU study handbook.
...
% TODO: confirm submitting institute at JKU (the thesis is supervised via the
% Medical University of Vienna but formally submitted at JKU).
...
\keywords{Geographic Atrophy, OCT, neural PDE solvers, MP-PDE, MM-PDE, moving mesh, graph neural networks, medical imaging, longitudinal prediction}
