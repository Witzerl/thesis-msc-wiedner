...
central vision loss in older adults in industrialised countries. Worldwide,
it ranks as the third most frequent cause of moderate-to-severe vision
impairment and the fourth most frequent cause of blindness
...
per year in the United States alone \citep{Singh2025,Vallino2024}. Unlike
other eye diseases that cause vision loss, such as cataract, glaucoma or
diabetic retinopathy, AMD is not counted among the avoidable causes of vision
loss, defined as those that known, cost-effective means can prevent or treat
\citep{Flaxman2020}. Clinical care therefore focuses on \emph{managing} the
progression of the disease once it has appeared.
...
inter-eye correlation \citep{Yehoshua2011,Boyer2017}. An ageing
population, frequent involvement of both eyes and eventual spread to the
fovea make GA a long-term disease that progresses differently in every
patient. The clinically relevant question is therefore not \emph{whether} a
patient will deteriorate, but \emph{how fast} and \emph{where} in the
macula the lesion will grow.
...
spatial configuration of the lesion at the future visit. This differs from
earlier deep-learning work on AMD, which predicts when an eye will convert to
a later disease stage \citep{Vogl2021}. The output here is not a single risk
score but a complete map of the lesion at the next visit.
...
that a model has to deal with. First, follow-up visits are
...
captures the GA lesion itself, while the surrounding ten retinal layer
boundaries encode the structural cascade in which the atrophy is embedded
...
contained in the lesion mask alone. Third, every patient carries \emph{covariates}, in particular age and
sex. Age is strongly associated with the risk of developing the disease
(\S\ref{sec:introduction:motivation}), and the evidence on sex is mixed;
whether either also helps to forecast where and how fast a lesion grows is
not established, and is tested in this thesis.

Meeting these requirements is not only a question of the model itself but
also of how the model is used. A typical image-to-image model maps one image
to another; on its own, it has no notion of elapsed time and no built-in way
to advance a state from one visit to the next. These properties have to come
from a framework around the model, one that conditions the prediction on
$\Delta t$ and rolls the state forward visit by visit. The next section
motivates such a framework, which borrows several of its techniques from
neural partial differential equation (PDE) solvers. Which model should sit
inside it is a separate question.
...
Neural PDE solvers turn these classical ideas into learned models.
Instead of discretising a known equation, they learn from data an operator
that maps the current state of a system, and the time that elapses, to its
next state. The literature offers many such operators, and they differ
mainly in their built-in assumptions: how far a single prediction can look
across the domain, whether it works on a regular grid or on a graph, and
whether physical terms such as transport are represented explicitly. For
GA, where the governing equation is unknown and only a few dozen patients
are available for training, these assumptions are likely to matter more
than model size.

Which of them GA progression actually requires cannot be decided by
testing a single architecture: if one model succeeds, the reason may be
its operator, its size, or the setup around it. This thesis therefore
separates the two. The setup --- how elapsed time enters the prediction,
how models are trained and how they are evaluated --- is fixed once, and
the operator is the only part that changes. Architectures from different
families can then be compared on equal terms. Unlike prior work, which has
remained anchored in cohort-level statistical atlases and time-to-conversion
survival models \citep{Vogl2021,Vallino2024}, every model in this comparison
forecasts the lesion of an individual eye directly on the imaging state.
...
\begin{quote}
  \itshape What is needed to forecast the progression of Geographic
  Atrophy from longitudinal OCT?
  \begin{enumerate}
    \item What kind of framework does the forecast need?
    \item What kind of operator does that framework need?
  \end{enumerate}
\end{quote}

The first part concerns everything that surrounds the model: how
irregular visit intervals enter the prediction, and how models are
trained and evaluated. The second concerns the model itself, the
operator that maps one visit to the next. Chapter~\ref{ch:method}
develops a framework that answers the first part and leaves the operator
as its only free component. Chapter~\ref{ch:experiments} then answers the
second by comparing different operators within that framework.


\section{Contributions (TODO: maybe remove)}
...
\begin{itemize}
  ...
  % NOTE (2026-07-26, see NOTES.md "Code-side sync"): the final DMM trains on a
  % single-channel SLO mask (scalar monitor), so this "multi-channel Frobenius-norm
  % monitor" contribution is stale — it was a superseded workaround, now removed.
  % Reframe the DMM contribution (SLO-mask train/inference-grid decoupling) before final.
...
Chapter~ef{ch:background} introduces GA and OCT imaging, the numerical
ideas that learned PDE solvers build on, and the families of learned models
considered in this work. Chapter~ef{ch:data} describes the Medical
University of Vienna GA cohort and how each visit is turned into an
eleven-channel state. Chapter~ef{ch:method} addresses the first part of
the research question by developing the framework, and describes the models
that fill its one free component. Chapter~ef{ch:experiments} addresses the
second part by comparing these models on equal terms.
Chapter~ef{ch:discussion} discusses what the results mean clinically and
methodologically, together with their limitations, and
Chapter~ef{ch:conclusion} concludes. The appendix reports an additional
experiment with an adaptive moving mesh.
