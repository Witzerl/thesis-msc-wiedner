...
cohort. The thesis therefore works on the OCT en-face grid.

A single OCT volume shows both the lesion and the retinal layers around it.
The lesion itself is
...
modelled at native voxel resolution. The clinical literature therefore
works with an \emph{en-face} image instead: a two-dimensional map of the
retina seen from the front, as in a fundus photograph, in which every pixel
corresponds to one A-scan of the volume and summarises what lies along it
\citep{Pilotto2015,Chu2022,Yehoshua2011,Vallino2024}. The present work
adopts the same convention.

Several ways of forming the en-face image from the volume are used in the
literature, for example summing the full depth of each A-scan
\citep{Yehoshua2011} or projecting only a thin slab around the RPE
\citep{Pilotto2015}. No projection is computed in this thesis. Instead, the MUW
data provide two kinds of en-face maps for every visit, on the same grid:
the GA lesion as a binary mask, and, for each of ten retinal layer
boundaries, its depth along every A-scan (\S\ref{sec:data:dataset}). These
maps are the imaging input the models in this thesis receive.
...
tractability of the models introduced in the subsequent chapters.
...
en-face grid: an indicator function that takes value one inside the GA
lesion and zero outside it. The only properties of channel~0 that this
chapter relies upon are that it is binary and that it is aligned to the
same en-face grid as the layer channels.
...
eye on the same horizontal grid as the lesion mask. The ten boundaries are ordered from the most superficial
to the deepest (\S\ref{sec:data:state}).
...
OCT volumes acquired in routine clinical practice.
% TODO: cite Mai2024 (Mai, Lachinov, Reiter, Riedl, Grechenig, Bogunovic &
% Schmidt-Erfurth, Ophthalmology Science 4(4):100466, 2024) once added to
% references.bib; replace the plain "Mai et al. (2024)" with \citep{Mai2024}.
The cohort comprises 553 volume scans over 75 eyes, acquired with a
Heidelberg Spectralis spectral-domain OCT device (Mai et al., 2024). Lateral pixel counts
